...
class AddDice
  ...
  module Node
    ...
    class DiceRoll
      ...
      def initialize(times, sides, critical, keep_drop=nil)
        @times = times.literal
        @sides = sides.literal
      ...
      def eval(randomizer)
        ...
        dice_ary = dice_groups.flatten
        total = if @keep_drop && @keep_drop.num
          dice_ary.sort!
          dice_ary.reverse! if @keep_drop.high?
          if @keep_drop.keep?
      ...
      def to_s
        if @critical
          "#{@times}D#{@sides}#{@keep_drop}@#{@critical}"
        else
          "#{@times}D#{@sides}#{@keep_drop}"
        end
      end
    ...
    class KeepDrop
      ...
      # ノードを初期化する
      # @param [Integer] literal 値
      def initialize(keep_drop_str)
        @literal = keep_drop_str
        @num = nil
      end

      # Keepする？
      def keep?
        @num && @literal.chars[0] == 'K'
      end

      # Dropする？
      def drop?
        @num && @literal.chars[0] == 'D'
      end

      # 高い方から？
      def high?
        @num && @literal.chars[1] == 'H'
      end

      # 低い方から？
      def low?
        @num && @literal.chars[1] == 'L'
      end

      # 文字列化
      def to_s
        @num ? @literal + @num.to_s : ''
      end
    end
  end
end
